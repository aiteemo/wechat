...
class WechatPay
{
    ...
    private $_result = '';
    private $_appid = '8888';                //微信分配的公众账号
    private $_mac_id = '8888';            //微信支付分配的商户号
    private $_device_info = '';        //终端设备号
    private $_pay_unifiedorder_path = "https://api.mch.weixin.qq.com/pay/unifiedorder";//统一下单接口
    private $_sandboxnew_pay_unifiedorder_path = "https://api.mch.weixin.qq.com/sandboxnew/pay/unifiedorder";//统一下单接口
    private $_getsignkey_path       = "https://api.mch.weixin.qq.com/sandboxnew/pay/getsignkey";//获取验签秘钥API
    ...
    public function unifiedOrder($paydata)
    {
        ...
        if ($res) {
            ...
            if ($data['return_code'] == 'SUCCESS') {
                ...
                $result['errcode'] = '307';
                $result['errmsg'] = '订单错误'.$data['return_code'];
                $result['data'] = $data;
            }
        } else {
    ...
    public function getKey()
    {
        ...
        //发送支付请求
        $res = $this->getHttpResponsePOST($this->_getsignkey_path,$xml);
        if($res)
        {
            $data = $this->xmlToArray($res);
            if($data['return_code']=='SUCCESS')
